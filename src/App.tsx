...
function App() {
  ...
  const [selectedBlockset, setSelectedBlockset] =
    useState<string>("overworld.bst");
  const [tileMap, setTileMap] = useState<number[][] | null>(null);
  const [selectedHeader, setSelectedHeader] =
    useState<string>("PalletTown.asm");
  // A debounced header state to avoid rapid re‑processing
  // const [debouncedHeader, setDebouncedHeader] =
  //   useState<string>(selectedHeader);
  const [mapHeader, setMapHeader] = useState<MapHeader | null>(null);

  // Cache for expensive data (constants and mappings)
  const [cachedConstants, setCachedConstants] = useState<Record<
    string,
    { width: number; height: number }
  > | null>(null);
  const [cachedMappings, setCachedMappings] = useState<Record<
    string,
    string
  > | null>(null);
    ...
    async function fetchConstantsAndMappings() {
      try {
        const [constRes, mappingRes] = await Promise.all([
          fetch(`${ASSETS_PATH}/constants/map_constants.asm`, {
            signal: controller.signal,
          }),
          fetch(`${ASSETS_PATH}/gfx/tilesets.asm`, {
            signal: controller.signal,
          }),
        ]);
  ...
  useEffect(() => {
    if (
      !mapHeader ||
      typeof mapHeader.width !== "number" ||
      typeof mapHeader.height !== "number"
    )
      return;
    const controller = new AbortController();
    async function loadMapData() {
      try {
        const [blkResponse, blocksetResponse] = await Promise.all([
          fetch(`${ASSETS_PATH}/maps/${selectedMap}`, {
            signal: controller.signal,
          }),
          fetch(`${ASSETS_PATH}/blocksets/${selectedBlockset}`, {
            signal: controller.signal,
          }),
        ]);
  ...
  // Create a ref to hold the latest header request ID
  const headerRequestIdRef = useRef(0);

  useEffect(() => {
    if (!cachedConstants || !cachedMappings) return; // Increment the request ID since we are starting a new header load
    ...
    async function initializeMap() {
      try {
        const headerResponse = await fetch(
          `${ASSETS_PATH}/headers/${selectedHeader}`,
          { signal: controller.signal }
        );
        ...
        if (headerMatch) {
          // Only update state if this response is from the most recent request
          if (currentRequestId !== headerRequestIdRef.current) {
            return; // an updated request has already begun, so ignore this response
          } // Parse constants and update state as before
      ...
      <div className="image-selector">
        ...
        {mapHeader && (
          <div className="map-info">
            ...
            {mapHeader.width !== undefined &&
              mapHeader.height !== undefined && (
                <p>
                    ...
                    <canvas
                      ...
                      ref={(canvas) => {
                        if (canvas && preloadedTilesets[selectedImage]) {
                          ...
                          if (ctx) {
                            const tilesPerRow =
                              preloadedTilesets[selectedImage].width / tileSize;
                            const srcX = (tileId % tilesPerRow) * tileSize;
                            const srcY =
                              Math.floor(tileId / tilesPerRow) * tileSize;
                            ctx.imageSmoothingEnabled = false;
                            ctx.drawImage(
